...
public class BannerController {
 ...
 return Result.error("请选择并上传活动展播封面图片！");
 }

 // 提取相对路径写入数据库
 banner.setImageUrl(aliyunOssService.toRelativePath(banner.getImageUrl()));

 Banner created = bannerService.createBanner(banner);
 if (created != null && org.springframework.util.StringUtils.hasText(created.getImageUrl())) {
 created.setImageUrl(aliyunOssService.toFullUrl(created.getImageUrl()));
 }
