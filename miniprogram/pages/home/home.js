...
Page({
 ...
 api.uploadImage(tempPath, 'banners/').then(uploadRes => {
 wx.hideLoading();
 // 保存相对路径（或 fullUrl），发布时提交给后端
 const imagePath = uploadRes.relativePath || uploadRes.fullUrl;
 this.setData({ 'publishBannerForm.imageUrl': imagePath });
 wx.showToast({ title: '已成功上传到阿里云', icon: 'success' });
 }).catch(err => {
 wx.hideLoading();
 wx.showToast({ title: '上传阿里云图片服务器失败', icon: 'none' });
 });
 }
 }
 });
