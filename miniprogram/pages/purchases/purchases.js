...
Page({
  ...
  openEnrollItemModal(e) {
    const item = e.currentTarget.dataset.item;
    if (!item) return;
    const userInfo = wx.getStorageSync('userInfo') || {};
    const parentDisplayName = userInfo.realName || userInfo.parentName || (userInfo.studentName ? userInfo.studentName + '家长' : '热心家长');
    this.setData({
      selectedItem: item,
      userParentDisplayName: parentDisplayName,
      enrollQuantity: '1',
      showEnrollItemModal: true
    });
  },

  closeEnrollItemModal() {
    this.setData({ showEnrollItemModal: false });
  },

  onQuantityInput(e) {
    this.setData({
      enrollQuantity: e.detail.value
    });
  },
